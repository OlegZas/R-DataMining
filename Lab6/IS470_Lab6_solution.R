...
# histogram of insurance expenses (1 pts)
hist(insurance$expenses)

# exploring relationships among all numeric variables: correlation matrix  (1 pts)
...
# 8. Build multiple linear regression model (1 pts)
lm_model2 <- lm(expenses~., data = datTrain)
summary(lm_model2)

# Make predictions on both training and testing sets  (1 pts)
prediction_on_train2 <- predict(lm_model2, datTrain)
prediction_on_test2 <- predict(lm_model2, datTest)

# Generating evaluation metrics on both training and testing data  (1 pts)
...
#Partition the dataset again (1 pts)
set.seed(1)
train_index <- createDataPartition(insurance$expenses, p=0.7, list=FALSE)
datTrain2 <- insurance[train_index,]
datTest2 <- insurance[-train_index,]

#Build multiple linear regression model and add interaction effects bmi30*smoker (1 pts)
lm_model3 <- lm(expenses~. + bmi30*smoker, data = datTrain2)
summary(lm_model3)
...
# 10-fold cv with M5P prediction method (1 pts)
df <- insurance
target <- 7
nFolds <- 10
seedVal <- 1
